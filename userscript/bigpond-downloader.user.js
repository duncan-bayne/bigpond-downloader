...
(function() {
  ...
  Bpd.BpdFile = (function() {
    ...
    function BpdFile(escapedUri) {
      if (!escapedUri) throw 'You must pass a URI when constructing a BpdFile.';
      this._uri = unescape(escapedUri).replace("bpd://", "http://");
      this.mp3Uris = [];
    }
  ...
  Bpd.MusicArchive = (function() {

    function MusicArchive(bpdFiles) {
      if (!bpdFiles) {
        throw 'You must pass a list of BPD files when constructing a MusicArchive.';
      }
      this.bpdFiles = bpdFiles.slice();
      this._zip = new JSZip();
    }

    MusicArchive.prototype.download = function(onSuccess) {
      return this._downloadBpdFiles(this.bpdFiles, this._sendBase64Bytes);
    };

    MusicArchive.prototype._downloadBpdFiles = function(bpdFiles, onSuccess) {
      var bpdFile,
        _this = this;
      bpdFile = bpdFiles.pop();
      if (bpdFile) {
        return bpdFile.download(function() {
          return _this._downloadBpdFile(bpdFile, function() {
            return _this._downloadBpdFiles(bpdFiles, onSuccess);
          });
        }, function() {});
      } else {
        return onSuccess(this._zip.generate());
      }
    };

    MusicArchive.prototype._downloadBpdFile = function(bpdFile, onSuccess) {
      var mp3Uris;
      mp3Uris = bpdFile.mp3Uris.slice();
      return this._downloadMp3Files(mp3Uris, onSuccess);
    };

    MusicArchive.prototype._downloadMp3Files = function(mp3Uris, onSuccess) {
      var mp3Uri,
        _this = this;
      mp3Uri = mp3Uris.pop();
      if (mp3Uri) {
        return this._downloadMp3File(mp3Uri, function() {
          return _this._downloadMp3Files(mp3Uris, onSuccess);
        });
      } else {
        return onSuccess();
      }
    ...
    MusicArchive.prototype._downloadMp3File = function(mp3Uri, onSuccess) {
      var _this = this;
      console.log("MusicArchive._downloadMp3File: downloading " + mp3Uri);
      return $.ajax(mp3Uri, {
        success: function(body) {
          var filename;
          filename = _this._mp3Filename(mp3Uri);
          _this._zip.add(filename, body);
          console.log("MusicArchive._downloadMp3File: downloaded and ZIPped " + filename);
          return onSuccess();
        },
        error: function() {
          return alert("" + mp3Uri + " could not be downloaded.");
        }
      });
    };

    MusicArchive.prototype._sendBase64Bytes = function(bytes) {
      return location.href = "data:application/zip;base64," + bytes;
    };

    MusicArchive.prototype._mp3Filename = function(mp3Uri) {
      return mp3Uri.match(/.*\/(.*mp3)/)[1];
    };
  ...
  Bpd.BigPondDownloader = (function() {
    ...
    BigPondDownloader.prototype._startsWith = function(str, token) {
      return str && str.indexOf(token, 0) >= 0;
    };
